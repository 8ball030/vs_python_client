    ...
    async def process_request(self, websocket, request_data: str, heartbeat: HeartBeat
        ...
        try:
            ...
            if req_id and not req_data:
                logger.warning(f"Received heartbeat: {req_id}")
                heartbeat.id = req_id
                # We need to check whether the hb has been signed by the broker
                return 
    ...
    try:
        ...
        async with websockets.connect(uri, max_size=5 * 2**20) as websocket:
            ...
            async def get_block_number(heartbeat):
                ...
                logger.debug(f"Block number: {heartbeat.block_number}")
                return heartbeat.block_number
            

            async def periodic_broker_ping(websocket, processor, heartbeat, interval: float = 5):
                while True:
                    try:
                        await get_block_number(heartbeat)
                        ...
                        if websocket.closed:
                            logger.warning("WebSocket connection closed, exiting ping task")
                            break
            ...
            try:
                ...
                async for message in websocket:
                    # Create a new task for each incoming request if not a heartbeat
...
def run(proxy_url: str, host: str, port: int, path: str, log_level: str):
