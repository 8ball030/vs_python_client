...
class AsyncRequestProcessor:
    ...
    async def process_request(self, websocket, request_data: str, heartbeat: HeartBeat) -> None:
            ...
            async with self.semaphore:
                ...
                try:
                    ...
                    response = await self.client.post(
                        self.proxy_url, json=req_data, headers={"Content-Type": "application/json"}
                    )
    ...
    async def _send_error_response(self, websocket, req_id: str, error_message: str) -> None:
        ...
        try:
            result = {"id": req_id, "error": error_message, "result": "error"}
            await websocket.send(json.dumps(result))
        except Exception as e:
    ...
    try:
        ...
        async with websockets.connect(uri) as websocket:
            ...
            async def on_broker_ping(*args, **kwargs):
                ...
                data = await websocket.send(json.dumps(heartbeat_data))
                print("Sent heartbeat to registration server" + str(data))
